<template>
  ...
  >
    ...
    <div
      v-if="helpText"
      class="absolute pin-r pin-b p-2"
    >
    ...
    <div class="overflow-hidden overflow-y-auto max-h-90px">
      <ul class="list-reset">
        ...
        >
          <component
            :is="item.label in partitionLabelLinkMap ? 'a' : 'span'"
            target="_blank"
            :href="`${item.label in partitionLabelLinkMap ? partitionLabelLinkMap[item.label] : '#'}`"
...
const colorForIndex = (index) => ['#F5573B', '#F99037', '#F2CB22', '#8FC15D', '#098F56', '#47C1BF', '#1693EB', '#6474D7', '#9C6ADE', '#E471DE'][index];
